...
1. 서브 클래스의 __init__()
    ...
    super()는 잘 생각해보면 슈퍼 클래스의 생성자라고 해석할 수 있을겁니다. potter = student()에서 보면 알 수 있듯이. 그러면 슈퍼 클래스의 객체가 .__init__(name)이라는 메서드를 호출했다고 해석할 수 있겠네요. 즉, 이상의 코드에서 Student 생성자의 호출이 완벽하게 마무리 되려면 super().__init__(name)에 의해서 슈퍼 클래스인 Person클래스의 인스턴스가 생성됩니다. 이후에 슈퍼 클래스의 인스턴스 변수인 name이 서브 클래스로 전달되고, 그 다음 서브 클래스에서 school 인스턴스 변수를 선언 및 초기화하여 서브 클래스의 인스턴스가 생성완료됩니다.
    
    : 생성자를 호출했다면 -> 객체가 생성되었다고 볼 수 있는데 -> 그렇다면 부모 클래스의 인스턴스와 자식 클래스의 인스턴스가 있는거 아니냐 -> 맞긴한데 그러면 별개의 인스턴스라고 봐야 하녀면 그게 또 조금 애매합니다.
    
    Java에서는 super() -> 얘는 생성자 / super.메서드명() 으로 super 자체가 객체인 경우가 있었지만 python에서는 supper()로 일원화되어 있습니다.
    
2. 서브 클래스의 인스턴스 자료형
    슈펴 클래스의 객체는 슈퍼 클래스의 인스턴스
    서브 클래스의 인스턴스는 서브 클래스의 인스턴스이면서 동시에 슈퍼 클래스의 인스턴스(부모 생성자 호출 했으니까)
    Student 클래스의 객체는 Student의 인스턴스이면서 Person의 인스턴스
    
    Java를 기준으로 instanceof 연산자 역할을 하는 것이 JS 에서는 typeof 연산자가 있었는데, python에서는 isinstance() 함수가 있습니다 -> 다 소문자입니다.
형식:
...
print(issubclass(Student, Person)) # 결과값 : True 객체 안만들고 클래스 간의 위계만 학인하고 싶으면 쓸 수 있겠습니다.
...
class Hybrid(Car):
    ...
    def charge_battery(self, charge):
        ...
        print(f'{charge}만큼 충전 했습니다.')

    def car_info(self):

        super().car_info()
        print(f'현재 충전 상태 : {self.charge} ')


hybrid = Hybrid(oil= 0, charge= 0)
hybrid.add_oil(50)
...
class Circle(Shape):
    def __init__(self, name, radius):
        super().__init__(name)
        self.radius = radius
        print(f'반지름이 {radius}인 {name}이 생성되었습니다. \n 이름이 {name}인 원의 넓이는 {Circle.area(self)}입니다.')
    def area(self):
...
class Rectangle(Shape):
    def __init__(self, name, width, height):
        ...
        self.width = width
        self.height = height
        print(f'너비가 {width},높이가 {height}인 {name}이 생성되었습니다. \n 이름이 {name}인 직사각형의 넓이는 {Rectangle.area(self)}입니다.')

    def area(self):
        return self.width * self.height

    def draw(self):
        print(Rectangle.area(self))
