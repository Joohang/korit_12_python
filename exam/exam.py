...
'''
phon_number_collector = True

while phon_number_collector:
    phon_number = input('전화번호를 입력하시오 >>> ')
    if phon_number == 'off':
        phon_number_collector = False
        print('프로그램 종료!')
    if len(phon_number)!=13:
        print(f'{phon_number} 유효하지 않은 전화번호 형식입니다.')
    else:
        print(f'전화번호의 중간 4자리는 {phon_number[4:8:]}')
