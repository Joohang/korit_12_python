...
n = int(input('후보자의 수를 입력하세요 >>> '))
for i in range(n):
    name = str(input(f'{i+1} 번째 후보자의 이름을 입력하세요 >>> '))
...
total_vote =int(input('전체 투표 횟수를 입력하세요 >>> '))
for i in range(total_vote):

    guide = ""
    for j in range(len(candidate)):
        guide += f'{j+1} : {candidate[j]}'
        if j<len(candidate)-1:
            guide += ', '
    num_of_candidate = int(input(f'{i + 1}번째 투표 ({guide}) >>> '))
    target_name = candidate[num_of_candidate-1]
    votes[target_name] += 1
print('--- 투표 결과 ---')
...
# for key in num_of_vote:
#     count = int(input(f'{key+1}번째 투표 {candidate1} >>> '))
