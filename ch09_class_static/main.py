...
# 객체 생성
person2 = Korean2()
person2.trip('일본')      # 객체명.클래스메서드() 호출도 가능하긴 합니다. 그런데 마찬가지로 권장되지 않습니다.
...
3. 정적 메서드 (static method)
    ...
    인스턴스를 생성하지 않아도 사용할 수 있음 - 클래스 메서드와의 공텅점 # 2
    
    특징 : 
        1) 인스턴스 / 클래스로 호출 가능 -> 클래스 매서드와의 공통점
        2) 생성된 인스턴스가 없어도 호출 가능 -> 클래스 메서드와의 공통점
        
        3) @staticmethod 데코레이터를 표기하고 작성 -> 클래스 메서드와의 차이점 # 1
        4) 반드시 작성해야 하는 매게변수(self . cls) 가 없음 -> 클래스 메서드와의 차이점 # 2
        
이상을 토대로 정적 메서드는 self/cls 둘 다 사용하지 않기 때문에 인스턴스 / 클래스 변수를 모두 사용하지 않는 메서드를 정의하는 경우에 적합합니다.
...
class Bag:
    ...
    def sell(cls):
        print('가방이 팔렸습니다.')
        cls.count -= 1      # 얘는 클래스 매서드니까 Bag.count가 아니라 cls.count입니다.
